...
def send_string(str,slp = s):
    """
    模拟键盘输入字符串
    """
    k.type_string(str, interval = slp)
...
def send_up(n = 1, interval = s):
    send_mykey(k.up_key,n,interval)
def send_down(n = 1, interval = s):
    send_mykey(k.down_key,n,interval)
def send_left(n = 1, interval = s):
    send_mykey(k.left,n,interval)
def send_right(n = 1, interval = s):
